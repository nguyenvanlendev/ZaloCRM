...
export async function parseAppointmentFromText(input: { orgId: string; text: string; now?: Date }): Promise<ParsedAppointment & { source?: 'ai' | 'fallback' } | null> {
  ...
  const system = [
    ...
    '- HOẶC từ khoá địa điểm: "tại [địa điểm]", "ở [địa điểm]", "[tên building/đường/quận]", "VP", "showroom", "căn hộ", "khóa học [name]"',
 ...
 * khóa học khóa học) → AI return {text, styles[]} format Zalo (bold/italic/
...
export async function aiFormatRichText(input: { orgId: string; rawText: string }): Promise<AiFormatResult> {
  ...
  try {
    ...
    // Test với đoạn khóa học 800 chars input → Gemini muốn trả ~7900 chars JSON ≈ 5000 tokens.
